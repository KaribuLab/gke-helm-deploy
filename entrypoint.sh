...
echo "$credentials_json" > ${WORKDIR}/credentials.json
echo "$CHART_VALUES" > values.deploy.yaml
gcloud config set disable_prompts true
gcloud auth activate-service-account --key-file=${WORKDIR}/credentials.json # Authenticate with the service account
...
if [ -z "$installed_chart" ]; then
    ...
    helm install ${CHART_NAME} ${CHART_PATH} -f ${WORKDIR}/values.deploy.yaml # Install the chart
else
    echo "Upgrading chart: ${CHART_NAME}"
    helm upgrade ${CHART_NAME} ${CHART_PATH} -f ${WORKDIR}/values.deploy.yaml # Upgrade the chart
fi
